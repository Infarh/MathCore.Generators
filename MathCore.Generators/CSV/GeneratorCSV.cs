...
using MathCore.Generators.MVVM;
...
{
    internal static readonly DiagnosticDescriptor CSV1001NoPartialClass = new(
        "CSV1001",
        "Class should partial",
        "Объявление класса должен быть с модификатором partial",
        "Usage",
        DiagnosticSeverity.Error,
        isEnabledByDefault: true,
        description: "Добавить partial"
    );

    internal static readonly DiagnosticDescriptor CSV1002StaticClass = new(
        "CSV1002",
        "Class must be no static",
        "Класс не должен быть статическим",
        "Usage",
        DiagnosticSeverity.Error,
        isEnabledByDefault: true,
        description: "Убрать static"
    );
...
""", Encoding.UTF8);
    ...
    private static void Execute(ClassDeclarationSyntax Class, Compilation Compilation, SourceProductionContext Context)
    {
        ...
        if (!Class.IsPartial())
        {
            Context.Report(CSV1001NoPartialClass, Class, Class.Keyword);
            return;
        }

        if (Class.IsStatic())
        {
            Context.Report(CSV1002StaticClass, Class.Modifiers.First(m => m.ValueText == "static"));
            return;
        }
        ...
        source
           ...
           .Append("// Partial class from {0}", Class.SyntaxTree.FilePath).LN()
           .Nullable()
           .Using("System")
           .Using("System.IO")
...
[ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(AddPartialToClassCodeFixProvider)), Shared]
public class AddPartialToClassCodeFixProvider : CodeFixProvider
{
    public override ImmutableArray<string> FixableDiagnosticIds { get; } = ImmutableArray.Create
    (
        GeneratorCSV.CSV1001NoPartialClass.Id
    );
...
[ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(RemoveStaticFromClassCodeFixProvider)), Shared]
public class RemoveStaticFromClassCodeFixProvider : CodeFixProvider
{
    public override ImmutableArray<string> FixableDiagnosticIds { get; } = ImmutableArray.Create
    (
        GeneratorCSV.CSV1002StaticClass.Id
    );
    ...
    public override async Task RegisterCodeFixesAsync(CodeFixContext context)
    {
        ...
        context.RegisterCodeFix(
            CodeAction.Create(
                "Убрать static",
                createChangedDocument: cancel => FixAsync(document, root, class_declaration, cancel),
                equivalenceKey: "Remove static"),
            context.Diagnostics);
    }

    private static Task<Document> FixAsync(Document Document, SyntaxNode Root, ClassDeclarationSyntax Class, CancellationToken Cancel)
    {
        var static_modifier = Class.Modifiers.First(m => m.ValueText == "static");

        var modifiers = Class.Modifiers.Remove(static_modifier);
